...
function ClubManagerCodingParticipantUserIdCreate() {
    ...
    const QuizUserdata = async (emailid) => {
        try {
            setError(false);
            SetIsLoading(true);
            const res = await fetch(`${baseUrl}/api/participant/coding/register/data/filter?search=${emailid}`, {
                method: 'POST',
                credentials: 'include',
            })
            if (res.status === 200) {
    ...
    const submidformdata = async (event) => {
        ...
        try {
            ...
            const res = await fetch(baseUrl + '/api/participant/coding/user-id/tech/form', {
                method: 'POST',
                body: formdata,
                credentials: 'include',
            })
            if (res.status === 200) {
                ...
                toast.success("Coding Participant Id Added");
            } else if (res.status === 401) {
                SetIsLoading(false);
                    ...
                    <form onSubmit={submidformdata} className='h-[auto]' encType='multipart/form-data' >
                        <div className="user-details">
                            ...
                            <div className="input-box">
                                ...
                                <input type="number" name='mobilenumber' value={Object(data).mobilenumber ? Object(data).mobilenumber : ''} required />
                            </div>
...
export default ClubManagerCodingParticipantUserIdCreate
