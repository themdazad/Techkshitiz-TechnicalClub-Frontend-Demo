...
const QuizLogin = () => {
    ...
    const [IsLoading, Error] = Data_Fetch_Api('/api/participant/quiz/auth/data');
    ...
    const datasubmit = async (event) => {
        ...
        if (Number(Object(getdata).userid) >= 0 && Number(Object(getdata).userid) <= 9999999999) {
            try {
                ...
                const res = await fetch(baseUrl + "/api/quiz/login", {
                    method: "POST",
                    headers: {
    ...
    const PermissionCheck = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                SetPermission(data.quiz_contest_login_permission);
                SetLoading(false);
            } else {
                SetPermission(1)
                SetLoading(false);
        ...
        return <>
            ...
            <div className='w-[100%] h-[100vh] bg-[#0a1722] flex justify-center items-center '>
                <div className='w-[1400px] p-[26px] max-[1350px]:w-[1000px] max-[1050px]:w-[800px] space-y-10'>
                    <h1 className=' uppercase text-[40px] font-[700] max-[1050px]:text-[30px] max-[800px]:text-[25px] text-center animate-pulse text-[#ffdd00] max-[480px]:text-[15px] max-[300px]:text-[12px]  '>We appreciate your participation in the Quiz competition hosted by the Technical Club of Gec Siwan</h1>
