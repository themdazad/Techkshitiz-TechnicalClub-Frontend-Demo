  ...
  return (
    <>
      <div className=" w-[100%] h-[60px] z-40 sticky top-0 bg-[#0c0f1e] border-b-[1px] border-[#2b3148] flex justify-center items-center">
        <div className={`w-[24%] select-none h-[auto] flex justify-center items-center max-xl:w-[90%] max-xl-[1280px]:pl-6  max-sm:pl-3 ${IsLogin ? "max-[700px]:w-[80%]" : ""} max-xl:justify-start`}>
        ...
        </div>
        {
          IsLogin ? (
            <div className="hidden  select-none w-[70px] pr-[10px] h-[100%] max-xl:flex justify-end items-center  ">
              <div onClick={mainheaderslidehandel} className={`  w-[45px] h-[45px] cursor-pointer    rounded-[50%] border-[2px] border-[#2c4056] flex justify-center items-center`}>
        ...
        >
          <ul className="w-[100%] h-[100%] max-[480px]:ml-[20px]   flex justify-center items-center ml:gap-6 mll:gap-8 gap-4 max-xl:gap-4  max-xl:overflow-y-auto  max-xl:w-[250px]  max-[480px]:w-[210px]  max-xl:block">
              ...
              >
                {getYear.map((element, index) => (
                  <div key={index} className=" p-[1px] pl-0 pr-0 text-[#fff] ">
                    <NavLink to={`/government-engineering-college-siwan/hall-of-fame?year=${element}`} className=" hover:text-[#f3ff46]  text-[16px]  flex justify-center items-center h-[35px] hover:bg-[#191e36e1] w-[100%]">{element}</NavLink>
                  </div>
                ))}
                <div className=" text-[#fff] p-[6px]  pl-0 pr-0 flex justify-center items-center  ">
                  <NavLink to='/government-engineering-college-siwan/hall-of-fame/?type=hall-of-fame&year=all' className=" hover:text-[#f3ff46]  flex justify-center items-center gap-2 h-[35px] text-[16px] hover:bg-[#191e36e1] w-[100%]">More Year  <span className="material-symbols-outlined text-[20px] ">
                    read_more
                  </span></NavLink>
...

            {
              IsLogin ? (
                <div
                  className="text-[#fff] max-xl:w-[100%] max-xl:flex max-xl:mt-[20px] justify-center max-xl:justify-start max-xl:items-start items-center  text-center text-[14px]   transition-all ease-in-out delay-200 font-[600] h-[auto] max-xl:h-[250px] pr-[10px] "
                >
                  <div onClick={OnClickprofileMenu} className="w-[45px] max-xl:hidden cursor-pointer  h-[45px] rounded-[50%] border-[2px] border-[#2c4056] flex justify-center items-center">
                    <img className="w-[45px] h-[45px] rounded-[50%] " src={Object(getdata).participant_Profile_Avtar ? Object(getdata).participant_Profile_Avtar : Object(getdata).participant_Avtar} alt="Avtar" />
                  </div>
                  <div id="profileMenu" className="xl-[1280px]:hidden  bg-[#0d0f1e] max-xl:pl-[10px] xl-[1280px]:absolute right-[30px] top-[60px] max-xl:w-[240px] w-[300px] h-[auto] rounded-[5px] border border-[#1a2941] p-[10px]  pl-[30px]">
                    <NavLink to={Object(getdata).participant_Roll === 0 ? "/intra/college/student/profile/" : Object(getdata).participant_Roll === 2 ? "/professor/profile/" : Object(getdata).participant_Roll === 3 ? "/club/manager/profile" : Object(getdata).participant_Roll === 3 ? "/club/manager/profile" : Object(getdata).participant_Roll === 4 ? "/campus/ambassador/profile" : "/student/profile/"} className="w-[100%] cursor-pointer h-[50px] flex gap-3 justify-start items-center hover:text-[#c84c4c] text-[#4f6b8f] ">
                      ...
                      <div className="">
                        <h4 className="text-[#fff] uppercase text-start text-[16px]    ">{Object(getdata).participant_Name.substring(0, 12)}</h4>
                        <p className=" underline  text-[12px] text-start  font-[500] ">{Object(getdata).techk_Shitiz_Id}</p>
                      </div>
                    </NavLink>
                    <div onClick={settingParticipantDetail} className=" hover:text-[#fff] text-[#e2e3e4] flex cursor-pointer justify-start items-center gap-3 w-[100%] h-[50px] pl-[10px]">
                      <span className="material-symbols-outlined">
                        settings
                      </span>
                      <p>Settings</p>
                    </div>
                    <div onClick={LogOutParticipant} className="hover:text-[#fff] text-[#e2e3e4] flex cursor-pointer justify-start items-center gap-3 w-[100%] h-[50px] pl-[10px]">
                      <span className="material-symbols-outlined">
                        logout
                      </span>
                      <p>Logout</p>
                ...
                >
                  <li className=" text-center  uppercase  h-[30px] flex items-center  max-xl:hidden ">Login</li>
                  <div
                    className="xl:hidden text-[yellow] w-[200px]   h-[auto] pb-[10px]"
                  >
                    <NavLink onClick={mainheaderslidehandel} to='/government-engineering-college-siwan/student/login' className=" hover:text-[#f3ff46]  text-[16px]  flex justify-start items-center h-[35px] mb-3  w-[100%]">Student Login  </NavLink>
                    <NavLink onClick={mainheaderslidehandel} to='/government-engineering-college-siwan/professor/login' className=" hover:text-[#f3ff46]  text-[16px]  flex justify-start items-center h-[35px]  w-[100%]">Professor Login</NavLink>
                  </div>
                  <div
                    id="login"
                  ...
                  >
                    <div className=" p-[1px] pl-0 pr-0 text-[#fff] ">
                      <NavLink to='/government-engineering-college-siwan/student/login' className=" hover:text-[#f3ff46]  text-[16px]  flex justify-center items-center h-[35px] hover:bg-[#191e36e1] w-[100%]">Student Login  </NavLink>
                    </div>
                    <div className=" p-[1px] pl-0 pr-0 text-[#ffff] ">
                      <NavLink to='/government-engineering-college-siwan/professor/login' className=" hover:text-[#f3ff46]  text-[16px]  flex justify-center items-center h-[35px] hover:bg-[#191e36e1] w-[100%]">Professor Login</NavLink>
                    </div>
